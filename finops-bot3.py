...
def init_redshift_client():
    ...
    client = MCPClient(lambda: stdio_client(
        StdioServerParameters(
            ...
            env={
                "AWS_DEFAULT_REGION": "us-east-1", # [주의] 실제 리전으로 변경 필요 (예: ap-northeast-2)
...
# 3. Agent 생성 함수 (간소화된 버전)
def create_agent(mcp_client):
    tools = mcp_client.list_tools_sync()
    
    # 효율적인 쿼리 작성을 위한 지침만 포함 (제한 로직 제거됨)
    system_prompt = f"""You are an AWS cost analysis assistant with access to Redshift.

🎯 GUIDELINE:
- **Try to answer the user's question using fewer than 5 queries.**
